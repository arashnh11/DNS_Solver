...
__global__ void PressureSolve(float * p_d, const float * p_old, const float * us_d, const float * vs_d, int p_xlength, int p_ylength, int wp, int wu, int wv, float dx, float dy, float dt)
{
...
__global__ void PressureBC(float * p_d, float * p_old, int nx, int ny, int dy, int wp)
{
    ...
    if (i > 0 && i < nx + 1 && j == 0){
        p_d[i * wp + j] = p_old[i * wp + j + 1]; // bottom wall - Final
    }
    if (i > 0 && i < nx + 1 && j == ny){
        p_d[i * wp + j] = p_old[i * wp + j - 1]; // Upper - no flux
    }
    if (j > 0 && j < ny + 1 && i == 0){
        p_d[i * wp + j] = p_old[(i + 1) * wp + j]; // left wall - not the inlet - Final
    }
    if (j > 0 && j < ny + 1 && i == nx && j*dy < 2.0){
        p_d[i * wp + j] = p_old[(i - 1) * wp + j]; // right wall - not the outlet - Final
    }
    if (j > 0 && j < ny + 1 && i == nx && j*dy >= 2.0){
        p_d[i * wp + j] = -p_old[(i - 1) * wp + j]; // pressure outlet - static pressure is zero - Final
    }
...
int main()
{
 ...
 //   thrust::host_vector<float> abs((nx + 1) * (ny + 1));
	...
	p_old = p;
            // SOR pressure solver
            PressureSolve<<< dim3( (ny+1)/BLOCK_SIZE + 1, (nx+1)/BLOCK_SIZE + 1, 1) , dim3(BLOCK_SIZE,BLOCK_SIZE,1)>>>(ptr_p, ptr_p_old, ptr_us, ptr_vs, p_xlength, p_ylength, wp, wu, wv, dx, dy, dt);
